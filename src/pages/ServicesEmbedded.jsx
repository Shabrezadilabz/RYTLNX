function ServicesEmbedded() {
  const capabilities = [
    { title: "Embedded Firmware Development", desc: "Bare-metal firmware, BSP, bootloaders, power management.", icon: "💾" },
    { title: "Real-Time Operating Systems (RTOS)", desc: "FreeRTOS, Zephyr, ThreadX, deterministic tuning.", icon: "⏱️" },
    { title: "Linux Systems Engineering", desc: "Kernel porting, drivers, middleware.", icon: "🐧" },
    { title: "System Integration & Optimization", desc: "Performance tuning, hardware-software co-validation.", icon: "🔧" },
    { title: "Testing & Automation", desc: "HIL, SIL, automated regression frameworks.", icon: "🧪" },
    { title: "Connectivity & Drivers", desc: "BLE, Wi-Fi, Ethernet, peripheral interfaces.", icon: "📡" }
  ];

  return (
    <main className="max-w-7xl mx-auto px-6 py-12 animated-bg">
      <div className="text-center mb-12 animate-fadeIn">
        <div className="inline-block px-4 py-1 bg-gradient-to-r from-indigo-100 to-purple-100 rounded-full text-sm font-medium text-indigo-700 mb-4">
          Services
        </div>
        <h1 className="text-4xl font-bold mb-3">
          Systems & <span className="gradient-text">Embedded Engineering</span>
        </h1>
        <p className="text-gray-600 text-lg max-w-3xl mx-auto">
          Robust embedded systems integrating firmware, RTOS, Linux, and system optimization.
        </p>
      </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {capabilities.map((cap, index) => (
            <div key={index} className="p-6 bg-gradient-to-br from-white to-gray-50 border border-gray-200 rounded-xl card-hover">
              <div className="w-14 h-14 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl mb-4 flex items-center justify-center text-2xl shadow-lg">
                {cap.icon}
              </div>
      ...
      <section className="glass rounded-2xl p-8 shadow-xl">
        ...
        <p className="text-gray-700 mb-6">
          We specialize in building reliable, performant embedded software that powers mission-critical applications.
        </p>
...
export default ServicesEmbedded;
