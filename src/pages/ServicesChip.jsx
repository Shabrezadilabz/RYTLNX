function ServicesChip() {
  const capabilities = [
    { title: "Architecture & Micro-architecture", desc: "Requirements analysis, system & block-level architecture, PPA feasibility.", icon: "🏗️" },
    { title: "RTL Design & Integration", desc: "IP/subsystem RTL, SoC integration, low-power techniques.", icon: "⚡" },
    { title: "Verification & Validation", desc: "UVM-based verification, coverage-driven validation.", icon: "✅" },
    { title: "Physical Design", desc: "Synthesis, place & route, timing closure and sign-off.", icon: "🔧" },
    { title: "DFT (Design For Test)", desc: "Scan insertion, ATPG, MBIST/LBIST.", icon: "🧪" },
    { title: "Silicon Bring-Up", desc: "Board bring-up, debug, characterization, yield optimization.", icon: "🚀" }
  ];

  return (
    <main className="max-w-7xl mx-auto px-6 py-12 animated-bg">
      <div className="text-center mb-12 animate-fadeIn">
        <div className="inline-block px-4 py-1 bg-gradient-to-r from-blue-100 to-purple-100 rounded-full text-sm font-medium text-blue-700 mb-4">
          Services
        </div>
        <h1 className="text-4xl font-bold mb-3">
          Chip & <span className="gradient-text">Silicon Engineering</span>
        </h1>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {capabilities.map((cap, index) => (
            <div key={index} className="p-6 bg-gradient-to-br from-white to-gray-50 border border-gray-200 rounded-xl card-hover">
              <div className="w-14 h-14 bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl mb-4 flex items-center justify-center text-2xl shadow-lg">
                {cap.icon}
              </div>
